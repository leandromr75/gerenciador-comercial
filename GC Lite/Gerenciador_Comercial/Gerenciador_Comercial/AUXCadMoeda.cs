...
{
    public partial class AUXCadMoeda : Form
    {
        public AUXCadMoeda()
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (textBox1.ReadOnly == false && String.IsNullOrEmpty(textBox1.Text) == false )
            {
                ...
                if (jaexiste == false)
                {
                    string Stored = "CriarCadMoeda";

                    DALCadastro.AUXCadCriar(Stored, textBox1.Text);
                    dataGridView1.DataSource = DALCadastro.AUXCadListar("ListarCadMoeda");
        ...
        private void AUXCadMoeda_Load(object sender, EventArgs e)
        {
            dataGridView1.DataSource = DALCadastro.AUXCadListar("ListarCadMoeda");
        }
        ...
        {
            if (String.IsNullOrEmpty(textBox1.Text) == false && textBox1.ReadOnly == true && textBox1.Text != "REAL R$")
            {
                string resul = Convert.ToString(DALCadastro.ProcuraAUXCad("ExcluiCadAUXMoeda", textBox1.Text));
                ...
                if (String.IsNullOrEmpty(resul) == true)
                {
                    ...
                    {
                        DALCadastro.ExcluiCadAUX("ExcluiMoeda", label3.Text);
                        dataGridView1.DataSource = DALCadastro.AUXCadListar("ListarCadMoeda");
                    }
                }
            }
